...
2. **Create base model** at `src/model/[entity]/` (see templates below)
3. **Create controller** at `src/server/controller/`
4. **Create service** at `src/server/service/`
...
- Directory: `src/model/[entity]/`

## 2. Create Base Model (`src/model/[entity]/`)
...
## 3. Create Controller (`src/server/controller/[entity]/`)
...
## 4. Create Service (`src/server/service/[entity]/`)
...
## 5. Register in Loader (`src/server/common/loader.js`)
...
Run model tests:
```bash
cd src/model/[entity]/_test && npm install && npm test
```

Run all tests to ensure nothing is broken:
```bash
cd src/model && npm test
cd src/model-server && npm test
cd src/server && npm run test:all
```

## See Also

- [Entity Creation Guide](../../docs/server/entity-creation-guide-js.md) - the full architectural reference
- [Creating Entities](../../docs/guide/creating-entities-js.md) - quick-start guide
- [Server Service Modules](../../docs/server/server-service-modules.md) - the service layer rules
- [Server Controller Modules](../../docs/server/server-controller-modules.md) - the controller layer rules
